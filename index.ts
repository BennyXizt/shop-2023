...
document.addEventListener("DOMContentLoaded", function () {
  const PATH: RequestInfo | URL = "http://localhost:3000";
  let c = new Controller(PATH);

  document.addEventListener("click", function (e) {
    const buttonAddToBacket = (e.target as HTMLElement).closest(
      ".products__item-add"
    );
    const buttonBacket = (e.target as HTMLElement).closest(
      ".header__navigate-link--basket"
    );
    const buttonWindow = (e.target as HTMLElement).closest(".window__hidden");
    const buttonRemoveFromBacket = (e.target as HTMLElement).closest(
      ".window__item-button--remove-product"
    );
    const buttonCreateProduct = (e.target as HTMLElement).closest(
      ".window__item-button--add-product"
    );
    const buttonAddNewProduct = (e.target as HTMLElement).closest(
      ".header__navigate-link--product"
    );
    const buttonToRegisterForm: HTMLElement = (e.target as HTMLElement).closest(
      ".authorise__item-label--link-signup"
    );
    const buttonToLoginForm: HTMLElement = (e.target as HTMLElement).closest(
      ".authorise__item-label--link-signin"
    );
    const buttonAuthentification: HTMLElement = (
      e.target as HTMLElement
    ).closest(".authorise__item-button");
    const buttonAuthentificationLogout: HTMLElement = (
      e.target as HTMLElement
    ).closest(".header__navigate-link--logout");
    const buttonPagination: HTMLElement = (e.target as HTMLElement).closest(".products__pagination-item")

    if (buttonAddToBacket) c.addToBacket(buttonAddToBacket);
    if (buttonBacket) c.showBacket(e);
    if (buttonWindow) c.closeWindow();
    if (buttonRemoveFromBacket) c.removeFromBacket(buttonRemoveFromBacket);
    if (buttonAddNewProduct) c.addNewProduct(e);
    if (buttonCreateProduct) c.buttonCreateProduct();
    if (buttonToRegisterForm) c.registerForm(e);
    if (buttonToLoginForm) c.loginForm(e);
    if (buttonAuthentification) c.authentification(e, buttonAuthentification);
    if (buttonAuthentificationLogout) c.authentificationLogout(e);
    if (buttonPagination) c.showPagination(buttonPagination);
  ...
  if (!document.cookie.includes("authorised=")) {
    const div: HTMLElement = document.querySelector(".authorise");
    div.style.display = "block";
    return;
  } else {
    const div: HTMLElement = document.querySelector(".authorise");
    const nav: HTMLElement = document.querySelector(".header__navigate-list");
    const child: HTMLElement = document.createElement("li");
    child.innerHTML = `
  ...
  if (document.cookie.includes("backet=")) {
    const backet: HTMLElement = document.querySelector(
      ".header__navigate-image"
    );
...
class Controller {
  ...
  startedPagination: number
  endedPagination: number
  

  constructor(path: RequestInfo | URL) {
  ...
  showPagination(e: HTMLElement) {
    

    const items = document.querySelector(".products__list");
    const paginationElement: HTMLElement = document.querySelector(".products__pagination")
    items.innerHTML = "";

    const filterBy = (
      document.querySelector(
        ".products__article-select--type"
      ) as HTMLSelectElement
    ).selectedOptions[0].value;

    const pagination = (
      document.querySelector(
        ".products__article-select--pagination"
      ) as HTMLSelectElement
    ).selectedOptions[0].value;

    let data = this.data.products.filter((e) => {
      if (filterBy == "All Products") return e;
      else return e.brand == filterBy;
    });
    ...
    if (pagination != "All Products") {
      const paginatedTotal: number = Math.round(data.length / parseInt(pagination))
      const paginationCounter: number = parseInt(e.innerText) % 3
  ...
  authentificationLogout(e: MouseEvent) {
    e.preventDefault();
  ...
  authentification(e: MouseEvent, curr: HTMLElement) {
    e.preventDefault();
    const type = curr.parentNode.parentNode.querySelector(
      ".authorise__subtitle"
    ).innerHTML;
    const login = (
      curr.parentElement.parentElement.querySelector(
        ".authorise__item-input--login"
      ) as HTMLInputElement
    ).value;
    const pass = (
      curr.parentElement.parentElement.querySelector(
        ".authorise__item-input--password"
      ) as HTMLInputElement
    ).value;
    ...
    if (type == "Authorise") {
      ...
      fetch(`${this.path}/users`)
        ...
        .then((data: IDataJSON) => {
          console.log(data.users);
          const foundData = data.users.find(
            (e) => e.login == login && e.password == pass
          );
          if (foundData) {
            const d = new Date();
  ...
  registerForm(e: MouseEvent) {
    e.preventDefault();
    const currForm: HTMLElement = document.querySelector(
      ".authorise__wrapper--signin"
    );
    const nextForm: HTMLElement = document.querySelector(
      ".authorise__wrapper--signup"
    );
  ...
  buttonCreateProduct() {
    const brand = (
      document.querySelector(".window__item-select--brand") as HTMLSelectElement
    ).selectedOptions[0].innerHTML;
    const color = (
      document.querySelector(".window__item-select--color") as HTMLSelectElement
    ).selectedOptions[0].innerHTML;
    const model = (
      document.querySelector(".window__item-input--model") as HTMLInputElement
    ).value;
    const price = (
      document.querySelector(".window__item-input--price") as HTMLInputElement
    ).value;
  ...
  addNewProduct(e: MouseEvent) {
    e.preventDefault();
    this.window.showAddNewProductWindow();
  }
  showBacket(e: MouseEvent) {
    e.preventDefault();

    const isBacketNotEmpty =
      document.cookie.split(" ").find((e) => e.startsWith("backet")).length > 8;
  ...
  removeFromBacket(e: Element) {
    if (!document.cookie.includes("backet=")) return;

    let text = e.parentElement.querySelector(".window__item-label").innerHTML;

    const foundElement = this.data.products.find(
      (e) => e.brand + " " + e.model == text
    );

    console.log(`Removed Element: ${JSON.stringify(foundElement)}`);
    let startIndexBacket: number = document.cookie.indexOf("backet=") + 7;
    let endIndexBacket: number =
      document.cookie.slice(startIndexBacket).lastIndexOf("}") + 1;
    let backetCookie: string = document.cookie.slice(
      startIndexBacket,
      startIndexBacket + endIndexBacket
    );
    const startIndexUser: number = document.cookie.indexOf("authorised=");
    const endIndexUser: number = document.cookie
      .slice(startIndexUser)
      .indexOf(";");
    const userCookie: string = document.cookie.slice(
      startIndexUser,
      startIndexUser + endIndexUser
    );
    ...
    endIndexBacket =
      document.cookie.slice(startIndexBacket).lastIndexOf("}") + 1;
    let div: any = document.querySelector(".header__navigate-counter");
    ...
    const cookie: Array<ICookieProductJSON> = backetCookie
      .split("|")
      .filter((e) => e != "")
      .map((e) => JSON.parse(e));
  ...
  addToBacket(elem: Element) {
    ...
    if (document.cookie.includes("backet=")) {
      const startIndexBacket: number = document.cookie.indexOf("backet=") + 7;
      const endIndexBacket: number =
        document.cookie.slice(startIndexBacket).lastIndexOf("}") + 1;
      let backetCookie: string = document.cookie.slice(
        startIndexBacket,
        startIndexBacket + endIndexBacket
      );
      const isProducNotExists: boolean =
        backetCookie.indexOf(JSON.stringify(foundElement)) == -1;
  ...
  loadData() {
    ...
    selectType.classList.add(
      "products__article-select",
      "products__article-select--type"
    );
    const selectPagination = document.createElement("select");
    selectPagination.classList.add(
      "products__article-select",
      "products__article-select--pagination"
    );
    ...
    selectType.addEventListener("change", function (e) {
      let filterBy: string = (e.target as HTMLInputElement).value;
      ref.filterData(filterBy);
    });
    selectPagination.addEventListener("change", function (e) {
      let paginated: string = (e.target as HTMLInputElement).value;
  ...
  paginatedData(pagination: string) {
    const items = document.querySelector(".products__list");
    const paginationElement: HTMLElement = document.querySelector(".products__pagination")
    items.innerHTML = "";
    const filterBy = (
  ...
  filterData(filterBy: string) {
    const items: HTMLElement = document.querySelector(".products__list");
    const paginationElement: HTMLElement = document.querySelector(".products__pagination")
    ...
    items.innerHTML = "";
    let data: Array<ICookieProductJSON> = this.data.products.filter((e) => {
      if (filterBy == "All Products") return e;
      else return e.brand == filterBy;
    });
...
class ModalWindow {
  ...
  showBacketWindow() {
    ...
    const startIndex = document.cookie.indexOf("backet=") + 7;
    const endIndex = document.cookie.lastIndexOf("}") + 1;
    const cookie: Array<ICookieProductJSON> = document.cookie
      .slice(startIndex, endIndex)
      .split("|")
      .map((e) => JSON.parse(e));

    if (cookie == void []) return;
